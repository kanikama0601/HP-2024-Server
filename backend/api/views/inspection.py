...
def inspection(request):
  ...
  if checkPermission(request.user, None, [PERMISSION_ADMIN, PERMISSION_INSPECTION]):
    ...
    if request.method == 'GET':
      ...
      count = len(news) + len(shop) + len(menu) + len(event) + len(organization_permission)

      return JsonResponse({'count': count, 'news': news, 'shop': shop, 'menu': menu, 'event': event, 'organization_permission': organization_permission})
...
def inspect(request, category, item_id):
    ...
    if request.method == 'GET':
      if not category in ['news', 'shop', 'menu', 'event', 'organization_permission']:
      ...
      return JsonResponse({'count': count, 'news': news, 'shop': shop, 'menu': menu, 'event': event, 'organization_permission': organization_permission, 'image': image})
