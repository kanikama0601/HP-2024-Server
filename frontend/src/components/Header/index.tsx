...
export const Header: React.FC = () => {
  ...
  const switchMenuOpen = () => setOpenMenu((v) => !v);
  ...
  const navLinkClass = "text-sm font-medium text-slate-600 hover:text-blue-700 transition-colors relative after:absolute after:bottom-[-2px] after:left-0 after:h-[2px] after:w-0 after:bg-blue-600 after:transition-all hover:after:w-full";

  return (
    <div className="w-full sticky top-0 z-40">
      <header
        className={`w-full px-4 py-3 md:px-8 flex items-center justify-between
          bg-white border-b border-blue-100
          transition-shadow duration-300
          ${scrolled ? 'shadow-[0_2px_20px_rgba(29,78,216,0.10)]' : 'shadow-none'}`}
      >
        {/* Logo */}
        <Link href="/" onClick={() => beginNavigation("/")}>
          <div className="flex items-center gap-2">
            <span className="inline-flex w-7 h-7 items-center justify-center bg-blue-700 rounded-md">
              <span className="text-white text-xs font-bold">電</span>
            </span>
        ...
        <nav className="hidden md:flex items-center gap-7">
          <Link href="/"       onClick={() => beginNavigation("/")}       className={navLinkClass}>Top</Link>
          <Link href="/news"   onClick={() => beginNavigation("/news")}   className={navLinkClass}>News</Link>
          <Link href="/shop"   onClick={() => beginNavigation("/shop")}   className={navLinkClass}>Shop</Link>
          <Link href="/event"  onClick={() => beginNavigation("/event")}  className={navLinkClass}>Event</Link>
          ...
          {isLoggedIn ? (
            <div className="relative">
              ...
              {openAccountMenu && (
                <div className="absolute right-0 mt-2 w-48 overflow-hidden rounded-xl border border-blue-100 bg-white shadow-lg shadow-blue-100/60">
                  <Link
                    href="/organization"
                    onClick={() => { beginNavigation("/organization"); setOpenAccountMenu(false); }}
                  ...
                  <Link
                    href="/logout"
                    onClick={() => { beginNavigation("/logout"); setOpenAccountMenu(false); }}
                    className="flex items-center gap-2 px-4 py-3 text-sm text-slate-600 hover:bg-blue-50 hover:text-blue-700 transition-colors"
                  >
            ...
            <div className="flex items-center gap-3">
              <Link href="/login"    onClick={() => beginNavigation("/login")}    className={navLinkClass}>Login</Link>
              <Link href="/register" onClick={() => beginNavigation("/register")} className="rounded-full bg-blue-700 px-4 py-1.5 text-sm font-medium text-white hover:bg-blue-800 transition-colors">Register</Link>
            </div>
          )}
        </nav>
        ...
        <>
          ...
          <nav
            className={`fixed right-0 top-0 h-screen w-72 z-40 bg-white border-l border-blue-100 shadow-xl
              ...
              {isLoggedIn ? (
                <>
                  <li>
                    <Link
                      href="/organization"
                      onClick={() => { beginNavigation("/organization"); setOpenAccountMenu(false); setOpenMenu(false); }}
                      className="flex items-center gap-3 px-3 py-3 rounded-xl hover:bg-blue-50 group transition-colors"
                    >
                      <span className="w-8 h-8 flex items-center justify-center rounded-lg bg-blue-50 group-hover:bg-blue-100 text-blue-600 transition-colors">
                  ...
                  <li>
                    <Link
                      href="/logout"
                      onClick={() => { beginNavigation("/logout"); setOpenMenu(false); }}
                ...
                <>
                  ...
                  <li>
                    <Link
                      href="/register"
                      onClick={() => beginNavigation("/register")}
                      className="flex items-center gap-3 px-3 py-3 rounded-xl hover:bg-blue-50 group transition-colors"
                    >
                      <span className="w-8 h-8 flex items-center justify-center rounded-lg bg-blue-50 group-hover:bg-blue-100 text-blue-600 transition-colors">
