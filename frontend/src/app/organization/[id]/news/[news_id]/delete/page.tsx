...
export default function News() {
  const { id, news_id } = useParams<{ id: string; news_id: string }>();

  const [loading, setLoading] = useState(false);
  const [status, setStatus] = useState(0);
  const url = import.meta.env.VITE_API_URL + `/organization/${id}/news/${news_id}/delete/`;
        ...
        <div className="relative z-10 container mx-auto px-4 text-center">
          <div className="flex items-center justify-center gap-2 mb-2">
            ...
            <h1 className="text-3xl font-bold tracking-[0.1em]">
              <FontAwesomeIcon icon={faTrashCan} className="mr-2 text-blue-300" />Delete News
            </h1>
          </div>
          <p className="text-sm text-blue-300 tracking-widest">お知らせを削除</p>
        </div>
      </div>
      {/* Content */}
      <div className="container mx-auto px-4 py-10 max-w-2xl">
        <div className="bg-white rounded-2xl p-6 md:p-8 border border-slate-100 shadow-sm">
          <form onSubmit={handleSubmit}>
            <p className="text-center text-slate-700 mb-6">お知らせを削除しますか？</p>
        ...
        <Link to={`/organization/${id}/news/${news_id}`} className="mt-6 flex items-center justify-center gap-1.5 text-sm text-blue-600 hover:text-blue-800 transition-colors">
          <FontAwesomeIcon icon={faChevronLeft} /> お知らせへ戻る
        </Link>
      </div>
    </main>
  );
